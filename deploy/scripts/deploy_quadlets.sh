...
DB_MIGRATION_PASSWORD=$(sudo podman exec flightctl-db printenv POSTGRESQL_MIGRATOR_PASSWORD 2>/dev/null || echo "migrator_password")
DB_APP_PASSWORD=$(sudo podman exec flightctl-db printenv POSTGRESQL_PASSWORD 2>/dev/null || echo "app_password")
...
export DB_ADMIN_PASSWORD=$(sudo podman exec flightctl-db printenv POSTGRESQL_MASTER_PASSWORD)
export DB_MIGRATION_USER=flightctl_migrator
export DB_MIGRATION_PASSWORD="$DB_MIGRATION_PASSWORD"

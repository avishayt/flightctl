...
-- Grant additional privileges needed for table/sequence operations
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO "${DB_MIGRATION_USER}";
...
-- Ensure migration user can create and manage indexes, constraints, and triggers
...
BEGIN
    ...
    LOOP
        IF obj.command_tag = 'CREATE TABLE' THEN
            EXECUTE format('GRANT SELECT, INSERT, UPDATE, DELETE ON %s TO "${DB_APP_USER}"', obj.object_identity);
        END IF;
        IF obj.command_tag = 'CREATE SEQUENCE' THEN
            EXECUTE format('GRANT USAGE, SELECT ON %s TO "${DB_APP_USER}"', obj.object_identity);
        END IF;
    END LOOP;
EXCEPTION
    WHEN OTHERS THEN

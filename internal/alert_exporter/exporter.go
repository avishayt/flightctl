...
func (a *AlertExporter) processingCycle(
	...
	newCheckpoint, err := eventProcessor.ProcessLatestEvents(processCtx, *checkpoint)
	processSpan.End()
	if err != nil {
	...
	// Calculate alert state changes
	a.calculateMetrics(metrics, *checkpoint, newCheckpoint, logger)
...
// calculateMetrics analyzes the differences between old and new checkpoints to calculate metrics
func (a *AlertExporter) calculateMetrics(metrics *ProcessingMetrics, oldCheckpoint, newCheckpoint *AlertCheckpoint, logger *logrus.Entry) {
	// Count old alerts
	oldAlertCount := 0
	for _, reasons := range oldCheckpoint.Alerts {
		oldAlertCount += len(reasons)
	}

	// Count new alerts
	newAlertCount := 0
	for _, reasons := range newCheckpoint.Alerts {
		newAlertCount += len(reasons)
	}

	metrics.AlertsCreated = newAlertCount - oldAlertCount
	if metrics.AlertsCreated < 0 {
		metrics.AlertsResolved = -metrics.AlertsCreated
		metrics.AlertsCreated = 0
	}
	metrics.ActiveAlerts = newAlertCount

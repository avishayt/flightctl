...
func (r *reconciler) reconcileSelectionDevices(ctx context.Context, orgId uuid.UUID, fleet *api.Fleet, key map[string]any, numToRender int) error {
	...
	for {
		...
		for _, d := range devices.Items {
			...
			// Emit FleetRolloutDeviceSelected event
			event := common.GetFleetRolloutDeviceSelectedEvent(ctx, lo.FromPtr(d.Metadata.Name), lo.FromPtr(fleet.Metadata.Name), templateVersionName)
			if event != nil {

...
func (h *ServiceHandler) CreateCertificateSigningRequest(ctx context.Context, csr api.CertificateSigningRequest) (*api.CertificateSigningRequest, api.Status) {
	...
	if err != nil {
		...
		h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.CertificateSigningRequestKind, *csr.Metadata.Name, status))
		return nil, status
	}
	...
	h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.CertificateSigningRequestKind, *csr.Metadata.Name, api.StatusCreated()))
...
func (h *ServiceHandler) PatchCertificateSigningRequest(ctx context.Context, name string, patch api.PatchRequest) (*api.CertificateSigningRequest, api.Status) {
	...
	if err != nil {
		status := StoreErrorToApiStatus(err, false, api.CertificateSigningRequestKind, &name)
		h.CreateEvent(ctx, GetResourceUpdatedEvent(ctx, api.CertificateSigningRequestKind, name, status, &updatedDesc))
		return nil, status
	}
	...
	h.CreateEvent(ctx, GetResourceUpdatedEvent(ctx, api.CertificateSigningRequestKind, name, api.StatusOK(), &updatedDesc))
	return result, api.StatusOK()
}
...
func (h *ServiceHandler) ReplaceCertificateSigningRequest(ctx context.Context, name string, csr api.CertificateSigningRequest) (*api.CertificateSigningRequest, api.Status) {
	...
	if err != nil {
		...
		if created {
			h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.CertificateSigningRequestKind, name, status))
		} else {
			h.CreateEvent(ctx, GetResourceUpdatedEvent(ctx, api.CertificateSigningRequestKind, name, status, &updatedDesc))
		}
		return nil, status

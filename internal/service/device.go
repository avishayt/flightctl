...
func (h *ServiceHandler) CreateDevice(ctx context.Context, device api.Device) (*api.Device, api.Status) {
	...
	h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.DeviceKind, *device.Metadata.Name, status))
...
func (h *ServiceHandler) ReplaceDevice(ctx context.Context, name string, device api.Device, fieldsToUnset []string) (*api.Device, api.Status) {
	...
	if created {
		h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.DeviceKind, name, status))
	} else {
		h.CreateEvent(ctx, GetResourceUpdatedEvent(ctx, api.DeviceKind, name, status, &updateDesc))
	}
	return result, status
}

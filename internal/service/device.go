...
func (h *ServiceHandler) ReadDevice(ctx context.Context, name string) (*api.Device, api.Status) {
	orgId := store.NullOrgId
...
func (h *ServiceHandler) DeleteDevice(ctx context.Context, name string) (*api.Device, api.Status) {
	orgId := store.NullOrgId

	err := h.store.Device().Delete(ctx, orgId, name, h.callbackManager.DeviceUpdatedCallback)
	return &api.Device{}, StoreErrorToApiStatus(err, false, api.DeviceKind, &name)
}

// (GET /api/v1/devices/{name}/status)
func (h *ServiceHandler) ReadDeviceStatus(ctx context.Context, name string) (*api.Device, api.Status) {
	orgId := store.NullOrgId

...
func (h *ServiceHandler) ReadFleet(ctx context.Context, name string, params api.ReadFleetParams) (*api.Fleet, api.Status) {
	orgId := store.NullOrgId
...
func (h *ServiceHandler) DeleteFleet(ctx context.Context, name string) (*api.Fleet, api.Status) {
	orgId := store.NullOrgId
	...
	if f.Metadata.Owner != nil {
		...
		return nil, api.StatusUnauthorized("unauthorized to delete fleet because it is owned by another resource")
	}

	err = h.store.Fleet().Delete(ctx, orgId, name, h.callbackManager.FleetUpdatedCallback)
	return &api.Fleet{}, StoreErrorToApiStatus(err, false, api.FleetKind, &name)
}

func (h *ServiceHandler) ReadFleetStatus(ctx context.Context, name string) (*api.Fleet, api.Status) {
	orgId := store.NullOrgId

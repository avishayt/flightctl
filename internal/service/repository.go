...
func (h *ServiceHandler) CreateRepository(ctx context.Context, repo api.Repository) (*api.Repository, api.Status) {
	...
	h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.RepositoryKind, *repo.Metadata.Name, status))
...
func (h *ServiceHandler) ReplaceRepository(ctx context.Context, name string, repo api.Repository) (*api.Repository, api.Status) {
	...
	if created {
		h.CreateEvent(ctx, GetResourceCreatedEvent(ctx, api.RepositoryKind, name, status))
	} else {
		h.CreateEvent(ctx, GetResourceUpdatedEvent(ctx, api.RepositoryKind, name, status, &updateDesc))
	}
	return result, status
}

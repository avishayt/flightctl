...
func (h *ServiceHandler) ReadRepository(ctx context.Context, name string) (*api.Repository, api.Status) {
	orgId := store.NullOrgId
...
func (h *ServiceHandler) DeleteRepository(ctx context.Context, name string) (*api.Repository, api.Status) {
	orgId := store.NullOrgId

	err := h.store.Repository().Delete(ctx, orgId, name, h.callbackManager.RepositoryUpdatedCallback)
	return &api.Repository{}, StoreErrorToApiStatus(err, false, api.RepositoryKind, &name)
}

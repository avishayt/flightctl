...
func (h *ServiceHandler) ReadResourceSync(ctx context.Context, name string) (*api.ResourceSync, api.Status) {
	orgId := store.NullOrgId
...
func (h *ServiceHandler) DeleteResourceSync(ctx context.Context, name string) (*api.ResourceSync, api.Status) {
	orgId := store.NullOrgId
	err := h.store.ResourceSync().Delete(ctx, orgId, name, h.store.Fleet().UnsetOwner)
	return &api.ResourceSync{}, StoreErrorToApiStatus(err, false, api.ResourceSyncKind, &name)
}

...
func (s *EventStore) InitialMigration() error {
	...
	// Create a composite index on involved_object_kind and involved_object_name
	if err := s.db.Exec("CREATE INDEX IF NOT EXISTS idx_event_involved_obj_kind_name ON events (involved_object_kind, involved_object_name)").Error; err != nil {

...
func (s *DataStore) RunMigrationWithMigrationUser(ctx context.Context, cfg *config.Config, log logrus.FieldLogger) error {
	ctx, span := instrumentation.StartSpan(ctx, "flightctl/store", "RunMigrationWithMigrationUser")
	defer span.End()

	// Create migration database connection
	migrationDB, err := InitMigrationDB(cfg, log.(*logrus.Logger))
	if err != nil {

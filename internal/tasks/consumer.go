...
func dispatchTasks(serviceHandler *service.ServiceHandler, store store.Store, callbackManager tasks_client.CallbackManager, k8sClient k8sclient.K8SClient, kvStore kvstore.KVStore) queues.ConsumeHandler {
...
func LaunchConsumers(ctx context.Context,
	...
	for i := 0; i != numConsumers; i++ {
		...
		for j := 0; j != threadsPerConsumer; j++ {
			if err = consumer.Consume(ctx, dispatchTasks(serviceHandler, store, callbackManager, k8sClient, kvStore)); err != nil {
				return err
			}
		}
	}

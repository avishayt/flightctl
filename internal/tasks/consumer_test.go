...
func TestDispatchTasks_WithNilMetrics_InvalidPayload(t *testing.T) {
	...
	// Mock consumer complete
	mockConsumer.On("Complete", mock.Anything, "entry-123", payload, mock.AnythingOfType("*json.SyntaxError")).Return(nil)
	...
	// Should return error but complete the message
	assert.Error(t, err)
	mockConsumer.AssertExpectations(t)
}

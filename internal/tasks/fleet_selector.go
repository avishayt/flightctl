...
func fleetSelectorMatching(ctx context.Context, event api.Event, serviceHandler service.Service, log logrus.FieldLogger) error {
...
func NewFleetSelectorMatchingLogic(log logrus.FieldLogger, serviceHandler service.Service, event api.Event) FleetSelectorMatchingLogic {
...
func (f FleetSelectorMatchingLogic) DeviceLabelsUpdated(ctx context.Context) error {
	f.log.Infof("Checking fleet owner due to device label update %s/%s", api.NullOrgId, f.event.InvolvedObject.Name)
...
func (f FleetSelectorMatchingLogic) FleetSelectorUpdated(ctx context.Context) error {
	startTime := time.Now()
	f.log.Infof("Checking fleet owner due to fleet selector update %s/%s", api.NullOrgId, f.event.InvolvedObject.Name)
...
func (f FleetSelectorMatchingLogic) handleFleetDeleted(ctx context.Context, allFleetsFetcher func() ([]api.Fleet, error)) error {
	...
	for {
		...
		for _, device := range devices.Items {
			...
			if err != nil {
				f.log.Errorf("failed to recompute ownership for device %s/%s: %v", api.NullOrgId, *device.Metadata.Name, err)
				errors++
			}
		}

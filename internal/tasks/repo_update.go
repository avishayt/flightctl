...
func repositoryUpdate(ctx context.Context, event api.Event, serviceHandler service.Service, log logrus.FieldLogger) error {
	logic := NewRepositoryUpdateLogic(log, serviceHandler, event)

	switch event.InvolvedObject.Kind {
	case api.RepositoryKind:
		err := logic.HandleRepositoryUpdate(ctx)
		if err != nil {
			log.Errorf("failed to notify associated resources of update to repository %s/%s: %v", api.NullOrgId, event.InvolvedObject.Name, err)
		}
	default:
		log.Errorf("RepositoryUpdate called with unexpected kind %s and op %s", event.InvolvedObject.Kind, event.InvolvedObject.Kind)
	}
	return nil
}
...
func NewRepositoryUpdateLogic(log logrus.FieldLogger, serviceHandler service.Service, event api.Event) RepositoryUpdateLogic {
	return RepositoryUpdateLogic{log: log, serviceHandler: serviceHandler, event: event}
}

...
func (h *TransportHandler) DeleteCertificateSigningRequest(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.DeleteCertificateSigningRequest(r.Context(), name)
	SetResponse(w, body, status)
}

// (GET /api/v1/certificatesigningrequests/{name})
func (h *TransportHandler) ReadCertificateSigningRequest(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadCertificateSigningRequest(r.Context(), name)
	SetResponse(w, body, status)
}

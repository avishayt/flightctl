...
func (h *TransportHandler) ReadDevice(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadDevice(r.Context(), name)
	SetResponse(w, body, status)
}
...
func (h *TransportHandler) DeleteDevice(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.DeleteDevice(r.Context(), name)
	SetResponse(w, body, status)
}

// (GET /api/v1/devices/{name}/status)
func (h *TransportHandler) ReadDeviceStatus(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadDeviceStatus(r.Context(), name)
	SetResponse(w, body, status)
}

...
func (h *TransportHandler) DeleteEnrollmentRequests(w http.ResponseWriter, r *http.Request) {
	...
	SetResponse(w, status, status)
}

// (GET /api/v1/enrollmentrequests/{name})
func (h *TransportHandler) ReadEnrollmentRequest(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadEnrollmentRequest(r.Context(), name)
	SetResponse(w, body, status)
}
...
func (h *TransportHandler) DeleteEnrollmentRequest(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.DeleteEnrollmentRequest(r.Context(), name)
	SetResponse(w, body, status)
}

// (GET /api/v1/enrollmentrequests/{name}/status)
func (h *TransportHandler) ReadEnrollmentRequestStatus(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadEnrollmentRequestStatus(r.Context(), name)
	SetResponse(w, body, status)
}
...
func (h *TransportHandler) PatchEnrollmentRequestStatus(w http.ResponseWriter, r *http.Request, name string) {
	status := api.StatusInternalServerError("not yet implemented")
	SetResponse(w, status, status)
}

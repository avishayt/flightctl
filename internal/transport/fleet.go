...
func (h *TransportHandler) ReadFleet(w http.ResponseWriter, r *http.Request, name string, params api.ReadFleetParams) {
	body, status := h.serviceHandler.ReadFleet(r.Context(), name, params)
	SetResponse(w, body, status)
}
...
func (h *TransportHandler) DeleteFleet(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.DeleteFleet(r.Context(), name)
	SetResponse(w, body, status)
}

// (GET /api/v1/fleets/{name}/status)
func (h *TransportHandler) ReadFleetStatus(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadFleetStatus(r.Context(), name)
	SetResponse(w, body, status)
}

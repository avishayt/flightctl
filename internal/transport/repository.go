...
func (h *TransportHandler) ReadRepository(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadRepository(r.Context(), name)
	SetResponse(w, body, status)
}
...
func (h *TransportHandler) DeleteRepository(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.DeleteRepository(r.Context(), name)
	SetResponse(w, body, status)
}

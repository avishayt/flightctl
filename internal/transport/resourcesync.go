...
func (h *TransportHandler) ReadResourceSync(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.ReadResourceSync(r.Context(), name)
	SetResponse(w, body, status)
}
...
func (h *TransportHandler) DeleteResourceSync(w http.ResponseWriter, r *http.Request, name string) {
	body, status := h.serviceHandler.DeleteResourceSync(r.Context(), name)
	SetResponse(w, body, status)
}

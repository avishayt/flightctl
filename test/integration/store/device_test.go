...
var _ = Describe("DeviceStore create", func() {
	...
	It("CreateOrUpdateDevice create mode race", func() {
		...
		_, created, err := devStore.CreateOrUpdate(ctx, orgId, &device, nil, true, nil, nil)
		Expect(err).ToNot(HaveOccurred())
	...
	It("CreateOrUpdateDevice update mode race", func() {
		...
		dev, created, err := devStore.CreateOrUpdate(ctx, orgId, &device, nil, true, nil, nil)
		Expect(err).ToNot(HaveOccurred())
	...
	It("CreateOrUpdateDevice update with stale resourceVersion", func() {
		...
		dev, _, err = devStore.CreateOrUpdate(ctx, orgId, dev, nil, false, nil, eventCallback)
		Expect(err).ToNot(HaveOccurred())
		Expect(called).To(BeTrue())

		dev.Spec.Os.Image = "newos"
		_, _, err = devStore.CreateOrUpdate(ctx, orgId, dev, nil, true, nil, eventCallback)
	...
	Context("Device store", func() {
		...
		It("Delete device success", func() {
			deleted, err := devStore.Delete(ctx, orgId, "mydevice-1", eventCallback)
			Expect(err).ToNot(HaveOccurred())
		...
		It("Delete device success when not found", func() {
			deleted, err := devStore.Delete(ctx, orgId, "nonexistent", eventCallback)
			Expect(err).ToNot(HaveOccurred())
		...
		It("CreateOrUpdateDevice update labels owned from API", func() {
			...
			_, _, err := devStore.CreateOrUpdate(ctx, orgId, &device1, nil, false, nil, eventCallback)
			Expect(err).ToNot(HaveOccurred())
			...
			_, _, err = devStore.CreateOrUpdate(ctx, orgId, &newDev, nil, true, nil, eventCallback)
		...
		It("UpdateOwner", func() {
			...
			dev.Metadata.Owner = lo.ToPtr("newowner")
			_, _, err = devStore.CreateOrUpdate(ctx, orgId, dev, nil, false, nil, eventCallback)
			Expect(err).ToNot(HaveOccurred())
			Expect(called).To(BeTrue())
			...
			_, _, err = devStore.CreateOrUpdate(ctx, orgId, dev, []string{"owner"}, false, nil, eventCallback)
			Expect(err).ToNot(HaveOccurred())
			Expect(called).To(BeTrue())
		...
		It("DeviceSpecsAreEqual integration scenarios", func() {
			...
			_, created, err := devStore.CreateOrUpdate(ctx, orgId, &device, nil, true, nil, eventCallback)
			Expect(err).ToNot(HaveOccurred())

...
var _ = Describe("EventStore Integration Tests", func() {
	...
	Context("Event Store", func() {
		It("List all events", func() {
			...
			Expect(eventList.Items[0].Reason).To(Equal(api.EventReasonResourceDeleted))
			Expect(eventList.Items[1].Reason).To(Equal(api.EventReasonResourceUpdated))
			Expect(eventList.Items[2].Reason).To(Equal(api.EventReasonResourceCreated))
		})
		...
		It("Filters events by actor", func() {
			...
			Expect(eventList.Items[0].Reason).To(Equal(api.EventReasonResourceUpdated))
			Expect(eventList.Items[1].Reason).To(Equal(api.EventReasonResourceCreated))
		})

...
var _ = Describe("EventStore Integration Tests", func() {
	...
	BeforeEach(func() {
		...
		if err := db.WithContext(ctx).Model(&model.Event{}).
			...
			Update("created_at", oneMinuteAgo).Error; err != nil {
	...
	Context("Event Store", func() {
		It("List all events", func() {
			listParams := store.ListParams{Limit: 100, SortColumn: lo.ToPtr(store.SortByCreatedAt), SecondarySortColumn: lo.ToPtr(store.SortByName), SortOrder: lo.ToPtr(store.SortDesc)}
		...
		It("List all events with paging", func() {
			listParams := store.ListParams{Limit: 2, SortColumn: lo.ToPtr(store.SortByCreatedAt), SecondarySortColumn: lo.ToPtr(store.SortByName), SortOrder: lo.ToPtr(store.SortDesc)}
			eventList, err := storeInst.Event().List(ctx, orgId, listParams)
			Expect(err).ToNot(HaveOccurred())
		...
		It("Filters events by reason", func() {
			listParams := store.ListParams{
				...
				SecondarySortColumn: lo.ToPtr(store.SortByName),

...
var _ = Describe("FleetRollout", func() {
	...
	When("the fleet is valid", func() {
		It("its devices are rolled out successfully", func() {
			...
			logic := tasks.NewFleetRolloutsLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.FleetKind, Name: *fleet.Metadata.Name}})
		...
		It("a new device is rolled out correctly", func() {
			...
			logic := tasks.NewFleetRolloutsLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.DeviceKind, Name: "mydevice-1"}})
			logic.SetItemsPerPage(2)

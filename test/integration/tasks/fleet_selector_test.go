...
var _ = Describe("FleetSelector", func() {
	...
	BeforeEach(func() {
		...
		logic = tasks.NewFleetSelectorMatchingLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.FleetKind, Name: "fleet"}})
	...
	Context("FleetSelector", func() {
		...
		It("Device labels updated with comprehensive event validation", func() {
			...
			for _, device := range devices.Items {
				deviceLogic := tasks.NewFleetSelectorMatchingLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.DeviceKind, Name: *device.Metadata.Name}})
		...
		It("Should skip updating decommissioning devices", func() {
			...
			_, _, err = deviceStore.CreateOrUpdate(ctx, orgId, device, nil, false, nil, nil)
			Expect(err).ToNot(HaveOccurred())
		...
		It("Should handle device with no labels in DeviceLabelsUpdated", func() {
			...
			deviceLogic := tasks.NewFleetSelectorMatchingLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.DeviceKind, Name: "no-labels-device"}})
			deviceLogic.SetItemsPerPage(2)
		...
		It("Should handle device with empty labels in DeviceLabelsUpdated", func() {
			...
			deviceLogic := tasks.NewFleetSelectorMatchingLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.DeviceKind, Name: "empty-labels-device"}})
			deviceLogic.SetItemsPerPage(2)
		...
		It("Should handle device with non-fleet owner in DeviceLabelsUpdated", func() {
			...
			deviceLogic := tasks.NewFleetSelectorMatchingLogic(log, serviceHandler, api.Event{InvolvedObject: api.ObjectReference{Kind: api.DeviceKind, Name: "non-fleet-owner-device"}})
			deviceLogic.SetItemsPerPage(2)
